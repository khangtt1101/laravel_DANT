    ...
    x-data="{
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div class="md:col-span-2 space-y-6">
                                ...
                                <template x-for="(address, index) in addresses" :key="address.id">
                                    <label class="flex items-center p-4 border rounded-md cursor-pointer">
                                        <input type="radio" name="user_address_id" :value="address.id"
                                               ...
                                               :checked="index === 0"> {{-- Tự động chọn địa chỉ đầu tiên --}}
                                        <div class="ml-3 text-sm">
                                            <p class="font-medium text-gray-900">{{ Auth::user()->full_name }}</p>
                                            <p class="text-gray-600" x-text="address.phone_number"></p>
                                            <p class="text-gray-600" x-text="`${address.address_line}, ${address.district}, ${address.city}`"></p>
                                        </div>
                                    </label>
                                </template>
                        ...
                        <div class="bg-white shadow rounded-lg p-6">
                            ...
                            <div class="space-y-4">
                                <label class="flex items-center p-4 border rounded-md cursor-pointer">
                                    <input type="radio" name="payment_method" value="COD" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500" checked>
                                    <span class="ml-3 text-sm font-medium text-gray-900">Thanh toán khi nhận hàng (COD)</span>
                                </label>
                                <label class="flex items-center p-4 border rounded-md cursor-pointer">
                                    <input type="radio" name="payment_method" value="MoMo" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500">
                                    <span class="ml-3 text-sm font-medium text-gray-900">Thanh toán qua MoMo</span>
                                </label>
                            ...
                            <ul class="divide-y divide-gray-200 mt-4">
                                ...
                                <li class="py-4 flex">
                                    ...
                                    <div class="ml-3 flex-1">
                                        ...
                                        <p class="text-xs text-gray-400">Giá: {{ number_format($details['price'], 0, ',', '.') }} đ</p>
                                    </div>
                                    <p class="text-sm font-medium text-gray-900">{{ number_format($details['price'] * $details['quantity'], 0, ',', '.') }} đ</p>
                                </li>
                                @endforeach
                            </ul>
        ...
        @else
            <div class="text-center py-12 bg-white shadow rounded-lg">
                ...
                <a href="{{ route('shop.index') }}" class="mt-6 inline-block bg-indigo-600 text-white font-medium py-3 px-8 rounded-md shadow-md hover:bg-indigo-700">
                    Tiếp tục mua sắm
                </a>
            </div>
        @endif
            ...
            <div @click.away="showAddressModal = false"
                ...
                <form @submit.prevent="addNewAddress">
                    <div class="p-6 space-y-4">
                        <div>
                            <label for="address_line" class="block text-sm font-medium text-gray-700">Địa chỉ (Số nhà, tên đường)</label>
                            <input x-model="newAddress.address_line" type="text" id="address_line" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required maxlength="255">
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label for="district" class="block text-sm font-medium text-gray-700">Quận / Huyện</label>
                                <input x-model="newAddress.district" type="text" id="district" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required maxlength="100">
                            </div>
                            <div>
                                <label for="city" class="block text-sm font-medium text-gray-700">Tỉnh / Thành phố</label>
                                <input x-model="newAddress.city" type="text" id="city" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required maxlength="100">
                            </div>
                        </div>
                        <div>
                            <label for="phone_number_modal" class="block text-sm font-medium text-gray-700">Số điện thoại</label>
                            <input x-model="newAddress.phone_number" type="tel" id="phone_number_modal" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required maxlength="15" {{-- Thêm maxlength --}}
                                   pattern="0[0-9]{9,10}"
