                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('shop.index') }}"
                                class="ml-1 md:ml-2 hover:text-indigo-600 transition-colors">Cửa hàng</a>
                        </div>
                    </li>
                    @if($product->category)
                        <li>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-0 lg:gap-12">
                    ...
                    <div class="p-6 lg:p-10 bg-white" x-data="{ 
                        ...
                        <div class="relative aspect-square rounded-xl overflow-hidden bg-gray-100 mb-6 group">
                            ...
                            <div class="absolute top-4 left-4">
                                <span
                                    class="bg-indigo-600 text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wide shadow-sm">Mới</span>
                            </div>
                        </div>
                        ...
                        @if($product->images->count() > 1)
                            <div class="grid grid-cols-5 gap-4">
                                @foreach($product->images as $image)
                                    <button @click="activeImage = '{{ Storage::url($image->image_url) }}'"
                    ...
                    <div class="p-6 lg:p-10 lg:pl-0 flex flex-col justify-center">
                        ...
                        <div class="mt-4">
                            <div class="flex items-center">
                                <div class="flex text-yellow-400">
                                    @for ($i = 1; $i <= 5; $i++)
                                        <svg class="h-5 w-5 {{ ($averageRating ?? 0) >= $i ? 'text-yellow-400' : 'text-gray-300' }}"
                        ...
                        @if(session('success'))
                            <div
                                class="mb-6 p-4 bg-green-50 border-l-4 border-green-500 text-green-700 rounded-r-md shadow-sm flex items-start">
                        ...
                        @if(session('error'))
                            <div
                                class="mb-6 p-4 bg-red-50 border-l-4 border-red-500 text-red-700 rounded-r-md shadow-sm flex items-start">
                                <svg class="w-5 h-5 mr-2 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div class="mt-auto" x-data="{ quantity: 1 }">
                            <div class="flex flex-col sm:flex-row gap-4">
                                <div class="w-fit">
                                    ...
                                    <div
                                        class="flex items-center border border-gray-300 rounded-lg overflow-hidden h-12">
                                        <button type="button" @click="if(quantity > 1) quantity--"
                                            class="w-10 h-full flex-shrink-0 flex items-center justify-center bg-gray-50 hover:bg-gray-100 text-gray-600 transition-colors focus:outline-none">
                                        ...
                                        <input type="number" id="quantity" x-model.number="quantity"
                                            class="w-12 h-full text-center border-0 focus:ring-0 text-gray-900 font-medium appearance-none"
                                            min="1" readonly>
                                ...
                                <button type="button" @click="$store.cart.addToCart({{ $product->id }}, quantity, true)"
                        ...
                        @foreach($relatedProducts as $item)
                            <a href="{{ route('products.show', ['category' => $item->category->slug ?? $item->category->id, 'product' => $item->slug ?? $item->id]) }}"
                                ...
                                <div class="relative h-48 bg-gray-50">
                                    ...
                                    @else
                                        <div class="w-full h-full flex items-center justify-center text-gray-400 text-sm">Chưa có
                                            ảnh</div>
                                    @endif
                                    <div class="absolute top-3 left-3">
                                        <span
                                            class="bg-indigo-600 text-white text-xs font-semibold px-3 py-1 rounded-full">HOT</span>
                                    </div>
                                </div>
                                <div class="p-4 space-y-2">
                                    <p class="text-xs font-medium text-indigo-600">{{ $item->category->name ?? 'Danh mục' }}</p>
                                    <h3 class="text-sm font-semibold text-gray-900 leading-tight line-clamp-2">{{ $item->name }}
                                    </h3>
            ...
            <div class="max-w-4xl mx-auto">
                ...
                @if($canReview)
                    <div class="mb-10 bg-gray-50 border border-gray-200 rounded-xl p-6"
                        x-data="{ selectedRating: {{ old('rating', 5) }} }">
                        ...
                        <form action="{{ route('products.reviews.store', $product) }}" method="POST" class="space-y-4">
                                    ...
                                    @for ($i = 1; $i <= 5; $i++)
                                        <label class="cursor-pointer">
                                            <input type="radio" name="rating" value="{{ $i }}" class="hidden"
                                                :checked="selectedRating == {{ $i }}" x-model="selectedRating">
                            ...
                            <div>
                                ...
                                <textarea name="comment" id="comment" rows="4"
                                    ...
                                    placeholder="Sản phẩm có tốt không? Bạn thích điều gì nhất?">{{ old('comment') }}</textarea>
                                @error('comment')
                                    <p class="text-sm text-red-500 mt-2">{{ $message }}</p>
                                @enderror
                            </div>
                            <button type="submit"
                                class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg font-medium hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                Gửi đánh giá
                            </button>
                        </form>
                    </div>
                @elseif(Auth::guest())
                    <div class="mb-10 bg-gray-50 border border-gray-200 rounded-xl p-6 text-sm text-gray-600">
                        Vui lòng <a href="{{ route('login') }}" class="text-indigo-600 font-medium">đăng nhập</a> để đánh
                        giá sản phẩm.
                    </div>
                @elseif($userReview)
                    <div class="mb-10 bg-white border border-gray-200 rounded-xl p-6"
                        x-data="{ selectedRating: {{ old('rating', $userReview->rating) }} }">
                        <h4 class="text-lg font-medium text-gray-900 mb-4">Chỉnh sửa đánh giá của bạn</h4>
                        <form action="{{ route('products.reviews.update', [$product, $userReview]) }}" method="POST"
                            class="space-y-4">
                            @csrf
                            @method('PUT')
                            <div>
                            ...
                            <div>
                                ...
                                <textarea name="comment" id="comment" rows="4"
                                    class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500"
                                    placeholder="Chia sẻ thêm về trải nghiệm của bạn">{{ old('comment', $userReview->comment) }}</textarea>
                                @error('comment')
                                    <p class="text-sm text-red-500 mt-2">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <form action="{{ route('products.reviews.destroy', [$product, $userReview]) }}" method="POST"
                            class="mt-4" onsubmit="return confirm('Bạn chắc chắn muốn xoá đánh giá này?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="inline-flex items-center px-4 py-2 bg-red-100 text-red-600 rounded-lg font-medium hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-red-200">
                                Xoá đánh giá
                            </button>
                        </form>
                    </div>
                            ...
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-base font-semibold text-gray-900">
                                        {{ $review->user->full_name ?? 'Người dùng' }}</p>
                                    <div class="flex text-yellow-400 mt-1">
                                        @for ($i = 1; $i <= 5; $i++)
                                            <svg class="h-4 w-4 {{ $review->rating >= $i ? 'text-yellow-400' : 'text-gray-300' }}"
                                                xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                                <path
                                                    d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L3.98 8.72c-.783-.57-.38-1.81.588-1.81H8.03a1 1 0 00.95-.69l1.07-3.292z" />
                                            </svg>
