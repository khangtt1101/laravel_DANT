            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-100">
                    <thead class="bg-slate-50/50">
                        <tr>
                            <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Mã Đơn</th>
                            <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Khách hàng</th>
                            <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Tổng tiền</th>
                            <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Trạng thái</th>
                            <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Ngày đặt</th>
                        ...
                        <tr class="hover:bg-slate-50/80 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <div class="text-sm font-medium text-slate-900">{{ $order->user->full_name ?? 'Khách vãng lai' }}</div>
                                </div>
                            </td>
                                ...
                                <form action="{{ route('admin.orders.update', $order) }}" method="POST" class="inline-block">
                                    ...
                                    <div class="relative">
                                        <select name="status" 
                                                ...
                                                class="appearance-none cursor-pointer pl-3 pr-8 py-1 rounded-full text-xs font-medium border-0 ring-1 ring-inset focus:ring-2 focus:ring-indigo-600 {{ $statusClasses[$order->status] ?? 'bg-slate-50 text-slate-700 ring-slate-200' }}">
                                            @foreach($statusLabels as $key => $label)
                                                <option value="{{ $key }}" {{ $order->status == $key ? 'selected' : '' }} class="bg-white text-slate-900">
                                                    {{ $label }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2">
                                            <svg class="h-3 w-3 opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                                        </div>
                                    </div>
                                </form>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{ route('admin.orders.show', $order) }}" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 px-3 py-1.5 rounded-lg transition-colors font-semibold text-xs uppercase tracking-wide">
                                    Chi tiết
                                </a>
                            </td>
                        </tr>
